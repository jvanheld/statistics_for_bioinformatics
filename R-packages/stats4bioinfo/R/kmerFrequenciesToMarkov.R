...
kmerFrequenciesToMarkov <- function(kmer.seq, 
  ...
  transition.matrix$Mean <- 2*count.matrix[, "Total"]/sumnum(count.matrix[,"Total"])
  row.names(transition.matrix) <- sub(row.names(transition.matrix), pattern = "Total", replacement = "Mean")
  transition.matrix["Mean", "Mean"] <- NA
