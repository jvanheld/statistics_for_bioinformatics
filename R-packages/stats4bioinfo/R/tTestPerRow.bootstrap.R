...
tTestPerRow.bootstrap <- function (x,
  ...
  verbose(paste(sep="", "tTestPerRow.bootstrap with ", iterations, " iterations. "), 1)
  ...
  for (i in 1:iterations) { 
    ...
    # table(cl)
    sample.ttpr <- tTestPerRow(x = x[,resampled.classes], cl = names(resampled.classes), ...)
  ...
  full.ttpr <- tTestPerRow(x = x, cl = cl, ...)
  ...
  result$groups <- full.ttpr$groups
  result$test.group <- full.ttpr$test.group
  result$control.group <- full.ttpr$control.group
